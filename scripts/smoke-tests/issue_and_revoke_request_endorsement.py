...
if __name__ == "__main__":
    ...
    params = {
        ...
        "alias": reservation_name,
        "create_transaction_for_endorser": "true",
    }
    res = requests.post(
        f"{PROXY_URL}/ledger/register-nym", headers=issuer_headers, params=params
    )
    register_nym_response = res.json()
    print(f"register_nym_response = {register_nym_response}")
    txn = register_nym_response["txn"]
    wait_a_bit()
    res = requests.get(
        f"{PROXY_URL}/transactions",
        headers=issuer_headers,
    )
    list_transactions_response = res.json()
    print(f"list_transactions_response = {list_transactions_response}")
    # grab first (and only) one...
    txn = list_transactions_response["results"][0]

    data = {"expires_time": "2030-03-31T05:22:19Z"}
    params = {
        "tran_id": txn["transaction_id"],
        "endorser_write_txn": "true",
    }
    res = requests.post(
        f"{PROXY_URL}/transactions/create-request",
        headers=issuer_headers,
        params=params,
        json=data,
    )
    create_transaction_response = res.json()
    print(f"create_transaction_response = {create_transaction_response}")
