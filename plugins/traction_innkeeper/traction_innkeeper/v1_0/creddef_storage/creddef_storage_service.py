...
class CredDefStorageService:
    ...
    def _extract_rev_reg_size(self, data: dict) -> Optional[int]:
        """Extract rev_reg_size from data dict, checking both direct key and options."""
    ...
    def _determine_support_revocation(self, cred_def_value: dict) -> bool:
        """Determine if revocation is supported from credential definition value."""
        return cred_def_value.get("revocation") is not None

    async def _fetch_anoncreds_cred_def(
        self, profile: Profile, cred_def_id: str, original_rev_reg_size: Optional[int]
    ) -> dict:
        """Fetch AnonCreds credential definition from registry and convert to storage format."""
    ...
    async def _fetch_indy_cred_def(
        self, profile: Profile, cred_def_id: str, data: dict
    ) -> dict:
        """Fetch Indy credential definition from ledger and update data dict."""
        async with profile.session() as session:
                ...
                try:
                    ...
                    if cred_def:
                        ...
                        if "support_revocation" not in data:
                            ...
                            data["support_revocation"] = (
                                self._determine_support_revocation(value)
                            )
    ...
    async def _enrich_cred_def_data(
        self,
        profile: Profile,
        cred_def_id: str,
        data: dict,
        original_rev_reg_size: Optional[int],
    ) -> dict:
        """Enrich credential definition data by fetching missing fields from registry or ledger."""
        ...
        if is_anoncreds:
            try:
                enriched_data = await self._fetch_anoncreds_cred_def(
                    profile, cred_def_id, original_rev_reg_size
                )
                self.logger.debug(f"anoncreds cred def = {enriched_data}")
                return enriched_data
            except Exception as err:
                self.logger.error(
                    f"Error fetching anoncreds credential definition: {err}"
                )
            ...
            try:
                return await self._fetch_indy_cred_def(profile, cred_def_id, data)
            except Exception as err:
    ...
    async def add_item(self, profile: Profile, data: dict):
        ...
        # Enrich data with missing fields from registry or ledger
        data = await self._enrich_cred_def_data(
            profile, cred_def_id, data, original_rev_reg_size
        )
