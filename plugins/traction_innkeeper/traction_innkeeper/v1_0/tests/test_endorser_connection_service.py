...
async def test_connect_with_endorser_create_connection(
    ...
    mock_tenant_mgr_instance.get_wallet_and_tenant = AsyncMock(return_value=(mock_wallet_rec, mock_tenant_rec))
    ...
    mock_didx_mgr_instance.create_request_implicit = AsyncMock(return_value=mock_didx_result)
    ...
    mock_tenant_mgr_instance.get_wallet_and_tenant.assert_awaited_once_with(TEST_TENANT_WALLET_ID)
...
def test_subscribe(endorser_service: EndorserConnectionService):
    ...
    mock_bus.subscribe.assert_called_once_with(CONNECTIONS_EVENT_PATTERN, connections_event_handler)
...
async def test_connections_event_handler_metadata_needs_setting(
    ...
    profile.session.assert_has_calls([call(), call()])  # Called twice: once for get_all, once for set
    ...
    mock_retrieve_by_id.assert_awaited_once_with(session, mock_conn_record.connection_id)
...
async def test_connections_event_handler_metadata_exists(
    ...
    mock_conn_record.metadata_get_all.return_value = {"transaction-jobs": {"transaction_my_job": "TRANSACTION_AUTHOR"}}
