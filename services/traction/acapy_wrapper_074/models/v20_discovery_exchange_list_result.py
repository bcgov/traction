# coding: utf-8

from __future__ import annotations
from datetime import date, datetime  # noqa: F401

import re  # noqa: F401
from typing import Any, Dict, List, Optional  # noqa: F401

from pydantic import AnyUrl, BaseModel, EmailStr, Field, validator  # noqa: F401
from acapy_wrapper_074.models.v20_discovery_exchange_list_result_results_inner import (
    V20DiscoveryExchangeListResultResultsInner,
)


class V20DiscoveryExchangeListResult(BaseModel):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.

    V20DiscoveryExchangeListResult - a model defined in OpenAPI

        results: The results of this V20DiscoveryExchangeListResult [Optional].
    """

    results: Optional[List[V20DiscoveryExchangeListResultResultsInner]] = Field(
        alias="results", default=None
    )


V20DiscoveryExchangeListResult.update_forward_refs()
