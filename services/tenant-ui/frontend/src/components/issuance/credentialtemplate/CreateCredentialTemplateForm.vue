    ...
    <div class="form-demo">
        <form @submit.prevent="handleSubmit(!v$.$invalid)">
            ...
            <div class="field">
                ...
                <small v-else-if="v$.name.$invalid && submitted" class="p-error">{{ v$.name.required.$message }}</small>
            </div>
            <!-- Tag -->
            <div class="field">
                <div class="p-float-label">
                    <InputText id="creddef_tag" v-model="v$.creddef_tag.$model"
                        :class="{ 'p-invalid': v$.creddef_tag.$invalid && submitted }" />
                    <label for="creddef_tag" :class="{ 'p-error': v$.creddef_tag.$invalid && submitted }">Credential Definition Tag*</label>
                </div>
                ...
                <small v-else-if="v$.creddef_tag.$invalid && submitted" class="p-error">{{ v$.creddef_tag.required.$message }}</small>
            </div>

            <!-- Revocation -->
            <div class="field">                
                <div class="field-checkbox">
                    <Checkbox inputId="creddef_revocation_enabled" 
            ...
            <div class="field">
                <div class="p-float-label">
                    <InputText id="creddef_revocation_registry_size" v-model="v$.creddef_revocation_registry_size.$model"
                        :class="{ 'p-invalid': v$.creddef_revocation_registry_size.$invalid && submitted }" />
                    <label for="creddef_revocation_registry_size" :class="{ 'p-error': v$.creddef_revocation_registry_size.$invalid && submitted }">Revocation Registry Size</label>
                </div>
                <span v-if="v$.creddef_revocation_registry_size.$error && submitted">
                    <span v-for="(error, index) of v$.creddef_revocation_registry_size.$errors" :key="index">
                        <small class="p-error">{{ error.$message }}</small>
                    </span>
                </span>
            </div>            
            
            <Button type="submit" label="Create" class="mt-1" :disabled="loading" :loading="loading" />
        </form>
    </div>
</template>
...
const selectedSchema = computed(() => {
    if (governanceStore.schemaTemplateDropdown != null) {
        return governanceStore.schemaTemplateDropdown.find((x) => x.value == props.schemaTemplateId);
    }
    return null;    
});
