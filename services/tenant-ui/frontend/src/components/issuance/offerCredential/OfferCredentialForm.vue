    ...
    <form @submit.prevent="handleSubmit(!v$.$invalid)">
      ...
      <div v-if="!showEditCredValues">
        ...
        <div class="field">
          ...
          <AutoComplete id="selectedCred" v-model="v$.selectedCred.$model" :disabled="credsLoading"
            :suggestions="filteredCreds" @complete="searchCreds($event)" :dropdown="true" optionLabel="label"
            forceSelection @change="resetCredValues" />
        ...
        <div class="field mt-5">
          ...
          <AutoComplete id="selectedContact" v-model="v$.selectedContact.$model" :disabled="contactLoading"
            :suggestions="filteredContacts" @complete="searchContacts($event)" :dropdown="true" optionLabel="label"
            forceSelection />
        ...
        <div class="field mt-5">
          ...
          <Textarea id="credentialValuesPretty" v-model="v$.credentialValuesPretty.$model"
            :class="{ 'w-full': true, 'p-invalid': v$.credentialValuesPretty.$invalid && submitted }" :autoResize="true"
        ...
        <Button type="submit" label="Send Offer" class="mt-5 w-full" :disabled="contactLoading || credsLoading"
      ...
      <div v-else>
        ...
        <div class="field mt-5">
          ...
          <Textarea id="credentialValuesEdit" v-model="v$.credentialValuesEditing.$model" :autoResize="true" rows="20"
            cols="50" class="w-full" />
...
const editCredentialValues = () => {
  ...
  if (!formFields.credentialValuesPretty.length) {
    ...
    const schemaId = credentialTemplates.value.find((ct: any) => ct.credential_template_id === formFields.selectedCred.value).schema_template_id;
    const schema = schemaTemplates.value.find((st: any) => st.schema_template_id === schemaId);
    schemaForSelectedCred.value = schema;

    const schemaFillIn = schemaForSelectedCred.value.attributes.map((a: string) => {
      return {
        name: `${a}`,
        value: ""
      }
    ...
    formFields.credentialValuesEditing = JSON.stringify(schemaFillIn, undefined, 2);
  }
}
