<template>
  ...
  >
    ...
    <Column :sortable="true" field="content" header="Content" filter-field="content">
      <template #filter="{ filterModel, filterCallback }">
        <InputText
	  v-model="filterModel.value"
	  type="text"
    ...
    <Column :sortable="true" field="created_at" header="Sent" filter-field="created_at">
      <template #body="{ data }">
        {{ data.created_at }}
      </template>
      <template #filter="{ filterModel, filterCallback }">
...
<script setup lang="ts">
 ...
 const formattedMessages: Ref<any[]> = computed(() => (messages.value.map((msg) =>
     ({
