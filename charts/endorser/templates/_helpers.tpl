...
*/}}
{{- define "endorser_api.ledgerBrowser" -}}
{{- $ledgerBrowser := dict "bcovrin-test" "http://test.bcovrin.vonx.io" -}}
{{ .Values.endorser_api.config.ledger.browserUrlOverride | default ( get $ledgerBrowser .Values.global.ledger ) }}
{{- end }}

{{/*
generate genesisfileurl
*/}}
{{- define "endorser_api.genesisUrl" -}}
{{ default (printf "%s%s" (include "endorser_api.ledgerBrowser" .) "/genesis") .Values.endorser_api.config.ledger.genesisUrlOverride }}
{{- end }}
...
curl -d '{\"seed\":\"$(WALLET_SEED)\", \"role\":\"TRUST_ANCHOR\", \"alias\":\"{{ include "acapy.fullname" . }}\"}' -X POST {{ include "endorser_api.ledgerBrowser" . }}/register;
{{- end -}}
{{- end -}}
...
{{/*
Create a default fully qualified endorser api name.
We truncate at 63 chars because some Kubernetes name fields are limited to this (by the DNS naming spec).
*/}}
{{- define "endorser_api.fullname" -}}
...
We truncate at 63 chars because some Kubernetes name fields are limited to this (by the DNS naming spec).
*/}}
{{- define "endorser_api.secret.name" -}}
{{ template "endorser_api.fullname" . }}
{{- end -}}

{{/*
Common endorser api labels
*/}}
{{- define "endorser_api.labels" -}}
{{ include "common.labels" . }}
{{ include "endorser_api.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
...
{{/*
Selector endorser api labels
*/}}
{{- define "endorser_api.selectorLabels" -}}
app.kubernetes.io/name: {{ include "endorser_api.fullname" . }}
{{ include "common.selectorLabels" . }}
{{- end }}

{{/*
Mount the endorser api config map as env vars
*/}}
{{- define "endorser_api.configmap.env.vars" -}}
envFrom:
  - configMapRef:
      name: {{ template "endorser_api.fullname" . }}
{{- end -}}

{{/*
Create the name of the endorser api service account to use
*/}}
{{- define "endorser_api.serviceAccountName" -}}
{{- if .Values.endorser_api.serviceAccount.create }}
{{- default (include "endorser_api.fullname" .) .Values.endorser_api.serviceAccount.name }}
{{- else }}
{{- default "default" .Values.endorser_api.serviceAccount.name }}
{{- end }}
{{- end }}

{{/*
generate endorser api hosts if not overriden
*/}}
{{- define "endorser_api.host" -}}
{{- include "endorser_api.fullname" . }}{{ .Values.global.ingressSuffix -}}
{{- end }}

{{- define "endorser_api.openshift.route.tls" -}}
{{- if (.Values.endorser_api.openshift.route.tls.enabled) -}}
tls:
  insecureEdgeTerminationPolicy: {{ .Values.endorser_api.openshift.route.tls.insecureEdgeTerminationPolicy }}
  termination: {{ .Values.endorser_api.openshift.route.tls.termination }}
{{- end -}}
{{- end -}}
